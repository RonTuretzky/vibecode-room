...
// substrate under PANOP_CUE_SOURCE_DIR — without cloning or building anything.
...
function freshTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "panop-cue-source-"));
  tempDirs.push(dir);
  return dir;
}

describe("cueSourceRoot / entrypoints — resolve under PANOP_CUE_SOURCE_DIR", () => {
  test("cueSourceRoot honours PANOP_CUE_SOURCE_DIR and the entrypoints hang off it", () => {
    process.env.PANOP_CUE_SOURCE_DIR = FIXTURE;
  ...
  test("cueSourceRoot falls back to a tmp cache dir when the env var is unset", () => {
    delete process.env.PANOP_CUE_SOURCE_DIR;
    expect(cueSourceRoot()).toBe(join(tmpdir(), "panopticon-cue-src"));
  });
});
...
describe("cueSourceBuildAvailable — gates harness vs fallback on build presence", () => {
  ...
  test("no build present (empty dir) reports unavailable", () => {
    process.env.PANOP_CUE_SOURCE_DIR = freshTempDir();
  ...
  test("a partial build (core dist present, server dist missing) reports unavailable", () => {
    ...
    process.env.PANOP_CUE_SOURCE_DIR = root;
