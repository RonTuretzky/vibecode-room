...
export function skylineProfile(height: number, year: number, ring: readonly number[], enabled: boolean): BuildingTier[] {
  if (!enabled || height < 45 || ring.length < 6 || ring.length > 40) return [{ top: 1, scale: 1 }];
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Many rectangular/chamfered NYC footprints contain extra collinear
    // survey vertices. Those should not silently disable the roof profile.
    if (Math.abs(cross) < 1e-6) continue;
    if (winding && Math.sign(cross) !== winding) return [{ top: 1, scale: 1 }];
    winding = Math.sign(cross);
  }
  if (!winding) return [{ top: 1, scale: 1 }];
