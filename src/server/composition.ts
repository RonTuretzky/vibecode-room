...
class LiveProjectorRuntime implements ProjectorRuntime {
  ...
  async selfBranches(): Promise<{ current: string; branches: Array<{ name: string; subject: string; date: string }> }> {
    ...
    // revert on every branch that carried the graft, renamed the whole rail
    // to 'Revert "self: …"' and the operator could no longer tell the
    // branches apart (live report: "the whole tree history is now reverts").
    // A room branch's identity is the SPOKEN graft it carries: the newest
    // `self:` commit on it. Reverts, merges and my own hand commits are
    // skipped; a branch with no self: commit at all keeps its tip subject
    // (honest — there is nothing better to call it).
    await Promise.all(
