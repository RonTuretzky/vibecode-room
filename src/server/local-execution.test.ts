...
test("local self-edit checks out isolated work and commits new files before advancing the clean room", async () => {
  ...
  await mkdir(join(repo, "artifacts/smithering"), { recursive: true });
  await writeFile(join(repo, "artifacts/smithering/evidence.json"), "original fixture");
  await writeFile(join(repo, "bun.lock"), "");
  await writeFile(join(repo, "package.json"), JSON.stringify({ scripts: { test: "bun checks.ts" } }));
  await writeFile(join(repo, "checks.ts"), `
    if (process.env.VIBERSYN_SELF_MODE) throw new Error("Leaked supervisor environment");
    await Bun.write("artifacts/smithering/evidence.json", "generated by test");
  `);
  await git(["add", "."]);
  await git(["commit", "-m", "seed"]);
  const t = new LocalExecutionTransport(
    { ...process.env, VIBERSYN_SELF_MODE: "1" },
    {
      roomRoot: repo,
  ...
  expect(await git(["show", "HEAD:artifacts/smithering/evidence.json"])).toBe("original fixture");
  expect(await git(["diff-tree", "--no-commit-id", "--name-only", "-r", "HEAD"])).toBe("index.html\nnew.js\n");
});

test("a failed self-check keeps the live room unchanged and retains the attempted edit", async () => {
  const repo = await root(), artifacts = await root();
  const git = (args: string[]) => runCommand(["git", "-c", "commit.gpgsign=false", ...args], repo, AbortSignal.timeout(15_000));
  await git(["init", "-b", "main"]);
  await git(["config", "user.name", "Room test"]);
  await git(["config", "user.email", "room-test@example.test"]);
  await writeFile(join(repo, "index.html"), "before");
  await writeFile(join(repo, "package.json"), JSON.stringify({ scripts: { typecheck: "bun -e 'throw new Error(\"intentional gate failure\")'" } }));
  await writeFile(join(repo, "bun.lock"), "");
  await git(["add", "."]); await git(["commit", "-m", "seed"]);
  const head = await git(["rev-parse", "HEAD"]);
  let isolated = "";
  const t = new LocalExecutionTransport(process.env, { roomRoot: repo, artifactsRoot: artifacts,
    agent: async out => { isolated = out; await writeFile(join(out, "index.html"), "attempted"); return "done"; } });
  await t.request("launchRun", { workflow: "vibersyn-self", input: { upid: "self", instruction: "change" }, options: { runId: "run-test" } });
  const run = await settled(t);
  expect(run.status).toBe("failed");
  expect(run.error).toContain("intentional gate failure");
  expect(await git(["rev-parse", "HEAD"])).toBe(head);
  expect(await readFile(join(repo, "index.html"), "utf8")).toBe("before");
  expect(await readFile(join(isolated, "index.html"), "utf8")).toBe("attempted");
}, 15_000);
