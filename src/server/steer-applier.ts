// ── STEER APPLIER (the stage guarantee) ─────────────────────────────────────
// The one-dwell demo beat: operator dwells the record toggle on an imported
// tree's branch, SPEAKS a change, toggles off — and a REAL commit with a sane
// diff lands on room/<slug> of builds/<upid>/repo, so the PR is never empty.
// This module makes the BOUNDED, REVIEWABLE edit that commit carries:
//   1. always append a dated entry to ROOM-NOTES.md at the repo root;
//   2. when the text names a recognizable target ("welcome"/"note"/"board"),
//      also insert one marked comment block + a visible <p class="room-note">
//      into dashboard/index.html IF it exists — bounded to ONE insertion,
//      idempotent per branch via the marker;
//   3. commit the working tree onto room/<slug> via
//      treeGit.commitBranchWithEdit — the fs edits above run INSIDE the
//      substrate's per-upid chain slot together with the commit
//      (detached-index plumbing — HEAD/checkout untouched, previews keep
//      serving), so a concurrent branch op (a PR ride's commit/push) can
//      never interleave between the edits and the commit that captures them,
//      and two rapid toggle cycles serialize their read-modify-write edits.
// Seams: fs is reached ONLY through the injectable repoDir root (tests use a
// tmpdir — no real clone, and the injected treeGit fake means no real git);
// the clock rides in via options. Honest results — {ok:false, error} on any
// fs or commit failure, never a throw.
//
// TODO(PATH A — agent-run steering): when an agent run replaces this direct
// applier, it slots exactly here — same inputs ({repoDir, branch, text,
// upid}), same commit contract — with applySteerEdit as the fallback so the
// stage guarantee (a real commit, never an empty PR) survives agent outages.
