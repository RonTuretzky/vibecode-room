...
export function RoomScene({ ideas, trees, mode, layout, wall = null, fitSignal, focusUpid = null, pointerNav = true, cornerLock = false, onAcceptIdea, onSelectProcess }: RoomSceneProps) {
  ...
  useEffect(() => {
    ...
    // Geometries/materials shared across ALL entries — never disposed per entry
    // (they live for the scene's lifetime and are freed once at unmount).
    const sharedGeo = new Set<THREE.BufferGeometry>(Object.values(GEO));
    const sharedMat = new Set<THREE.Material>([trunkMat, stemMat]);

    // Dispose an entry and every per-entry GPU resource it owns. Rings, satellite
    // pips, progress arcs and beacons all allocate their own geometry/material,
    // so sweep the whole group (deduped, skipping the shared pools) rather than
    // relying on entry.mats alone.
    const disposeEntry = (entry: Entry) => {
      ...
      entry.group.traverse((node) => {
        ...
        if (node instanceof THREE.Mesh || node instanceof THREE.Points) {
          if (!sharedGeo.has(node.geometry)) {
