...
export function placeSceneCards(cards: readonly ScreenCard[], width: number, height: number): PlacedCard[] {
  ...
  for (const card of ordered) {
    ...
    const x = Math.max(margin + card.width / 2, Math.min(width - margin - card.width / 2, card.x + (card.preferred?.dx ?? 0)));
    if (Math.abs(x - card.x) > 72) continue;
    let bottom = Math.min(card.y + (card.preferred?.dy ?? 0), height - 64);
    // Moving upward past an intersecting card is monotonic and bounded by
    // the number already placed. There is no iterative force simulation.
    for (let pass = 0; pass <= placed.length; pass++) {
      ...
      bottom = Math.min(...overlaps.map(other => other.y + other.dy - other.height - gap));
    }
    if (bottom - card.height < margin || card.y - bottom > Math.min(160, height * .28)) continue;
    placed.push({ ...card, dx: x - card.x, dy: bottom - card.y });
  }
  return placed;
}

export interface SceneCard { id: string; label: THREE.Sprite; priority: number }
...
export function createSceneCardLayout() {
  ...
  const reset = () => {
    for (const [label, original] of records) {
      label.scale.copy(original.base); label.center.copy(original.center); label.visible = true;
      delete label.userData.sceneCardRect;
    }
    records.clear(); lastCards = []; geometry.setDrawRange(0, 0); lines.visible = false;
  };
  return { group, reset, update(camera: THREE.PerspectiveCamera, width: number, height: number, cards: SceneCard[]) {
    const live = new Set(cards.map(card => card.label));
    for (const label of records.keys()) if (!live.has(label)) records.delete(label);
    lastCards = cards;
    ...
    for (const { id, label, priority } of cards) {
      ...
      const pixelWidth = Math.min(Math.min(220, width - 32), Math.max(priority > 0 ? 120 : 0, naturalWidth));
      ...
      screen.push({ id, x: (endpoint.x + 1) * width / 2, y: (1 - endpoint.y) * height / 2,
        width: pixelWidth, height: pixelHeight, depth: -view.z, priority, preferred: priority > 0 ? original.offset : undefined });
      anchors.set(id, anchor.clone());
    }
    const placed = placeSceneCards(screen, width, height);
