...
# http(s)://<room-ip>:<port+1|+2>/hands on their own computer or phone and
# drives the wall — webcam hand-tracking (pinch to click; runs in THEIR
# browser), a trackpad fallback, and held WASD buttons for the room camera.
# The wall's 🖐 Guests button pops the URL as a QR code. A self-signed TLS
# cert is generated automatically (browsers only allow the webcam on https);
# add &remote=0 to a wall URL to opt that window out. (--guests is accepted
...
# ── Guest hands: self-signed TLS for webcam hand-tracking (always on) ────────
