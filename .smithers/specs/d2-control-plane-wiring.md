...
## Goal
Make each Vibersyn Process a DURABLE SMITHERS RUN (the D1 foundation), so the
...
- **MetaSession boots the app gateway** (`startAppGateway`) on start and holds a `SmithersControlPlane`.
  Provide a clean shutdown (close gateway) and make the port configurable (VIBERSYN_GATEWAY_PORT, default 7332).
...
- **Do not double-drive:** the autonomy tick must NOT also run `brain.step()` for control-plane-backed
  ...
  (keep the old in-proc path only behind VIBERSYN_OFFLINE or for mock processes if needed).
...
- **Tests (bun test, no mocks):** add an integration test that boots a MetaSession (gateway on port 0),
  ...
  Gate behind VIBERSYN_SMOKE_AGENT=1 like the durable-run smoke (it spawns a real agent step).
- `bun run typecheck` AND `bun test` pass; the gated integration test passes with
  `VIBERSYN_SMOKE_AGENT=1 bun test --timeout 180000`.

## Constraints
- APP code only under `src/`. Do NOT touch `vibersyn-world/` (separate workstream), `node_modules/`,
...
- Event translation fidelity (Smithers run events → Vibersyn EventBus) — research the streamRunEvents
