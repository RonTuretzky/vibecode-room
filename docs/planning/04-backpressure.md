...
| AC6.4 | Each routed utterance produces a distinct audible ack: fed-idea-engine vs steer-X vs ignored (`observe.pass`) | unit_test | blocking | `bun test src/audio/output-policy.test.ts` (acks-distinct) | Block merge; the four Layer-B acks must be pairwise distinct | RBG: make ambient pass silent without a `prdAmendment:'AC6.4-silence'` flag → AC6.4-compliance test fails; passing test (suggestion/steer/addressed-pass/ambient-pass acks pairwise distinct); ambient-knob refused absent amendment flag |
...
| Criterion ID | Criterion | Method | Gate | Checked by | Failure action | Evidence required |
|---|---|---|---|---|---|---|
| AC11.1 | Default (Safe) posture runs to completion without per-step approval | integration_test | blocking | `bun test src/safety/shell-classifier.test.ts` + `src/safety/mode.test.ts` (safe-mode-autonomy) | Block merge; Safe must not collapse into Explicit | RBG: move `ls`/gate all shell → an approval appears on `bun test`/`git status`/`grep` → fails; passing safe-shell (read-safe ungated) + safe-mode-autonomy (read-safe sequence → 0 approvals); e2e Safe run autonomous; depends P-SHELL-PARSE, P-HOOK |
| AC11.2 | Before any destructive/irreversible action, the process speaks a read-back and waits for spoken "confirm"; absent confirm, no execution | integration_test | blocking | `bun test src/safety/classifier.test.ts` (hook-intercept) | Block merge; a mutating tool must be held, not executed | RBG: bypass the hook → file modified before approval → fails; passing hook-intercept test (mutating/`unknown` tool held, file unmodified while held, `ApprovalRequest` emitted); e2e safety-roundtrip; depends P-HOOK |
| AC11.3 | Dead-man timer (20–30 s) armed on dangerous acts: a missed/mis-heard "stop" still aborts, not executes | integration_test | blocking | `bun test src/safety/classifier.test.ts` (posture state-machine) | Block merge; timeout must abort the held tool | RBG: let the action fire without confirm → fails; passing state-machine test (no "confirm" in 25 s → abort, tool not executed, mocked clock); fuzz: garbled/double/wrong-gate confirm → never double-execute |
| AC11.4 | "Dangerous" (gate off) and fully-"explicit" modes are opt-in by voice and off by default | unit_test | blocking | `bun test src/safety/mode.test.ts` | Block merge; non-default modes must require warned voice opt-in | RBG: default a fresh process to `dangerous` → fails; passing mode-default (fresh = safe), dangerous/explicit only after spoken warning+confirm, dangerous resets to safe at session end |
...
| AC16.3 | From traces alone (no live system), an engineer can reconstruct any utterance's full observation→decision→action→outcome chain | integration_test | blocking | `bun test src/obs/trace.test.ts` (causal-chain reconstruction) | Block merge; an unreconstructable chain fails | RBG: drop `correlationId` join key → chain cannot be rebuilt → fails; passing reconstruction test (rebuild full chain incl. safety read-back→resolution from recorded traces); e2e: reconstruct from persisted traces, assert matches live run |
...
| P-HOOK | Smithers PreToolUse hook can intercept-and-hold a destructive tool call before execution; approve/deny/timeout resolve (file unmodified on deny/timeout) | integration_test | blocking | `bun test poc/p-hook.test.ts` (real Smithers run) | Block build; REQ-11 unenforceable until green — revisit safe-executor/posture at gate | RBG probe (held tool does not execute; deny/timeout leaves file unmodified); recorded report |
