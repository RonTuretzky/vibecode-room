#!/usr/bin/env python3
"""Fetch the pinned CC0 bark maps, or verify local copies with --check."""
import argparse
import hashlib
import json
...
ROOT = Path(__file__).resolve().parents[1] / "public/assets/park/bark"
...
def main():
    ...
    for maps in manifest.values():
        for source in maps.values():
            ...
            if not valid:
                if args.check:
                    raise ValueError(f"Missing or changed bark map: {path.name}")
